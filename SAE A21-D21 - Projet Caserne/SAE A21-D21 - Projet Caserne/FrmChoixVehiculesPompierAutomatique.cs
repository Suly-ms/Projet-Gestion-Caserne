...
namespace SAE_A21_D21___Projet_Caserne
{
    public partial class FrmChoixVehiculesPompierAutomatique : Form
    {
        ...
        {
            // Affichage des pompiers
            foreach (DataRow row in m_dataSet.Tables["Pompiers"].Rows)
            {
            ...
            foreach (DataRow row in m_dataSet.Tables["Vehicules"].Rows)
            {
                string type = row["Type"].ToString();
                int numero = Convert.ToInt32(row["Numero"]);
                int caserne = Convert.ToInt32(row["Caserne"]);
